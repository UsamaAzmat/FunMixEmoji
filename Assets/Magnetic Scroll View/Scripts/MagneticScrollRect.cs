...
namespace MagneticScrollView
{
    ...
    public class MagneticScrollRect : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        public static event Action onScrollingEvent;
        /// <summary>
        ...
        [SerializeField, Range(-1000f, 1000f)] private float m_elementPadding = 0f;
        ...
        public bool isScrollingEnded = false;
        ...
        public void ScrollBack()
        {
            ...
            // SoundsManager.instSound.PlayTheSound(soundType.shuffling, true);
        ...
        /// Auto Scroll Method
        /// </summary>
        public void StartAutoScroll(float duration = 1.25f)
        {
            // Ensure that auto-scrolling is not already in progress.
            if (!isScrolling && !isAutoScrolling && elements.Length > 0)
            {
                //autoScrollDuration = /*Random.Range(*/1.2f/*)*/;
                autoScrollDuration = 1.25f;
        ...
        private IEnumerator AutoScrollCoroutine()
        {
            ...
            // Calculate the total number of frames for the auto-scrolling duration.
            int totalFrames = Mathf.RoundToInt(autoScrollDuration / Time.unscaledDeltaTime);
            // Calculate the angle change per frame to scroll for 1 minute.
            float angleChangePerFrame = 180.0f / totalFrames;

            for (int frame = 0; frame < totalFrames; frame++)
            {
                // Increment the ScrollAngle each frame to simulate auto-scrolling.
                ScrollAngle += angleChangePerFrame;

                // Update the circular positions for the elements.
                for (int i = 0; i < elements.Length; i++)
                {
                    CircularPosition(elements, radius, i);
                }

                // Invoke any onScrolling events if needed.
                onScrolling.Invoke();
                onScrollingEvent?.Invoke();
                yield return null;
            }

            // Ensure the ScrollAngle ends at a multiple of 180 degrees for symmetry.
            ScrollAngle %= 180f;

            // Perform any additional tasks once auto-scrolling is finished (if needed).
            int rand = UnityEngine.Random.Range(0, elements.Length);
            InstantScrollTo(rand);
            if (TutorialHandler.Instance.indexer == 1) MergeManager.Instance.leftScroll.InstantScrollTo(rand);
            if (TutorialHandler.Instance.indexer == 2) MergeManager.Instance.rightScroll.InstantScrollTo(rand);
        ...
        public void OnEndDrag(PointerEventData ped)
        {
            ...
            if (TutorialHandler.Instance && TutorialHandler.Instance.indexer == 3 && TutorialHandler.isTutorial)
                TutorialHandler.Instance.showNextTutorial(1.5f);
            else if (TutorialHandler.Instance && TutorialHandler.Instance.indexer == 4 && TutorialHandler.isTutorial)
                TutorialHandler.Instance.showNextTutorial(1.5f);
        }
